...
* [Data-mining](#data-mining)
    ...
    * [Modification et ou nettoyage des données](#modification-et-ou-nettoyage-des-données)
        ...
        * [Opération sur les dataframes](#opération-sur-les-dataframes)
          ...
          * [Remplacer NA par 0](#remplacer-na-par-0)
...
packages <- c("ggplot2", "plyr", "gtools", "dplyr", "ggridges", "stringr", "RColorBrewer",
              ...
              "cowplot", "RAM", "VennDiagram", "datasets", "corrgram")
ipak(packages)
```
...
Ou directement si ils sont déjà nommer
```{r facteur_4}
distribution_courrier <- factor(c(5,0,10,5,5,0), levels = c(0,5,10))
distribution_courrier
distribution_courrier <- ordered(distribution_courrier,
                         levels = c(0,5,10), labels = c("rapide", "moyen", "fort"))
distribution_courrier
```
...
##### Merge deux dataframes

##### Merge plusieurs dataframes

Avec R on peu merge/fusionner plusieurs dataframe en séléctionnant des colonnes. Pour par exemple les comparers, au delà de 2 c'est moins facile à réaliser

```{r}
mergeABCD <- Reduce(function(x, y) merge(x, y, by = c("pos", "ALT"), all=TRUE), list(T24, T48, T60, T72))
```

#### Coller les colonnes de deux dataframes

La fonction `cbind()` pour "column bind" permet de coller 2 dataframes "l'un à coté de l'autre". Il est obligatoire d'avoir le **même nombre de ligne** dans les 2 dataframes.

```{r cbind}
donnees <- data.frame(A=c(18,17,17,17,12,5,10,8), B=c(8,12,15,10,1,5,20,7), C=c(18,15,16,14,13,17,15,15))
donnees_2 <- data.frame(A=c(18,17,17,17,12,5,10,8), B=c(8,12,15,10,1,5,20,7))
cbind(donnees, donnees_2)
```


#### Coller les lignes de deux dataframes

La fonction `rbind()` pour "row bind" permet de coller 2 dataframes "l'un sous l'autre". Il est obligatoire d'avoir le **même noms de colonne** dans les 2 dataframes. Il existe un moyen si le nombre de colonne n'est pas le même avec `rbind.fill()`

```{r rbind}
donnees_3 <- data.frame(A=c(18,17,17), B=c(8,12,15), C=c(1,2,3))
rbind(donnees, donnees_3)
```


```{r rbind_fill}
donnees_2
rbind.fill(donnees, donnees_2)
```
...
```{r}
noms_long <- "~/Documents/private_AF11-I-T48_intersec_3_replicats.csv"
Name_Var <- deparse(titute(noms_long)) 
Name_Var
```

#### Concaténer un vecteur caractère en chaine de caractère

```{r}
vecteur_str <- c("A","C", "T", "G","A")
paste0(vecteur_str)
paste0(vecteur_str, collapse = "")
```


#### Manipuler des sequences d'ADN

https://web.stanford.edu/class/bios221/labs/biostrings/lab_1_biostrings.html

##### Transformer des chaines de caractère en sequences d'ADN 

Il faut pour cela les stocker dans un objet `DNAString`

```{r}
# Transformer des chaines de caractère en ADN
b <- DNAString(paste0(share_SNPs_Atl$REF, collapse = ""))
c <- DNAString(paste0(share_SNPs_Atl$A_ALT_T72_R3, collapse = ""))
```


##### Générer des sequences d'ADN aléatoire

Ici la taille des sequences est de 10

```{r}
paste(sample(DNA_ALPHABET[1:4], 10, replace=T), collapse="")
```
...
Ce qui revient à afficher le 100% de la fonction quantile.
...
Ce qui revient à afficher le 0% de la fonction quantile.
...
## Faire des sous ensembles pour faire des plots

Une fois que l'on a un dataframe/matrice correctement formaté, voir la partie [datamining](#data-mining), on peut vouloir faire des analyses sur différents sous ensembles. Il existe alors 2 possibilités : 

- Crée un nouveau dataframe en séléctionnant le sous-groupe de données puis faire le graphe du nouveau dataframe.

- Faire directement le graphe en sélectionnant des sous-ensemble directement !

La deuxième solution étant plus élégante et pratique il existe 2 manières de procédé si les données sont numérique ou des caractères. Bien évidemment on peut réaliser plusieurs opérations avec les opérateur logique de R.
...
```{r}
donnees_4 <- data.frame(gene_un=c(185,178,175,171,172 ,165, 170, 171, 164, 182 ), gene_deux=c(82,69,70,65,68, 70, 71, 67, 63, 80), souris=c("wt","wt","wt","m","m","wt","wt","m","m","m"))
donnees_4

stripchart(gene_un ~ souris, data = donnees_4,
...
donnees_4 <- data.frame(gene_un=c(185,178,175,171,172 ,165, 170, 171, 164, 182 ), gene_deux=c(82,69,70,65,68, 70, 71, 67, 63, 80), souris=c("wt","wt","wt","m","m","wt","wt","m","m","m"))
donnees_4
donnees_4$souris <- as.factor(donnees_4$souris)

# Stripcharts basiques
ggplot(donnees_4, aes(x=souris, y=gene_un, color=souris)) + 
...
Pour plus d'information allé cette [article](http://www.sthda.com/french/wiki/ggplot2-stripchart-jitter-guide-de-demarrage-rapide-logiciel-r-et-visualisation-de-donnees).

## Boxplot

Les boxplots sont une représentation simplifié du stripchart. C'est un moyen rapide de décrire un variable. On l'utilise couramment pour comparer un même caractère dans deux échantillons.

```{r}
box_plot <- ggplot(donnees_4, aes(x=souris, y=gene_un, color=souris)) + 
  geom_boxplot()+
  labs(title="Plot of gene expression of one gene per mices",x="mices", y = "gene one")
box_plot + theme_classic()

#box_plot + stat_summary(fun.y=mean, geom="point", shape=23, size=4) # ajout de la moyenne
```

Plus d'infos sur cette [article](http://www.sthda.com/french/wiki/ggplot2-box-plot-guide-de-demarrage-rapide-logiciel-r-et-visualisation-de-donnees)
...
Un moyen simple de faire un venne diagram est fusionner 2 dataframe par une (ou plusieurs) colonne(s) en utilisant la fonction `merge()`. Puis supprimer les lignes ou il y a des `NA` qui ont été introduit avec la fonction `complete.cases()`. Il reste uniquement les lignes qui sont commune aux deux dataframes.
Il ne reste plus qu'a utiliser le nombre de ligne pour avoir les chiffres qui rempliront le venn diagram.

```{r venn_dia_1}
donnees
donnees_3

mergeAB <- merge(donnees, donnees_3, by = c("A", "B"), all = TRUE) # fusion des dataframes, NA si pas de fusion possible
intersectionAB <- mergeAB[complete.cases(mergeAB), ] # Intersection des 2 dataframes
A <- nrow(donnees)
B <-nrow(donnees_3)
AB <- nrow(intersectionAB)
grid.newpage()
...
```{r}
subset(env_ATL_clean, !(grepl('AM',env_ATL_clean$ENV)) & grepl("*ORF*",env_ATL_clean$ORF))
```

On utilise `!(grepl())` pour faire la négation de grep
...
## Multi plot avec les mêmes légendes

https://stackoverflow.com/questions/47614314/how-to-put-plots-without-any-space-using-plot-grid
https://stackoverflow.com/questions/40854737/multiple-top-textgrob-titles
...
Voir pour le coverage :
https://davetang.org/muse/2015/08/05/creating-a-coverage-plot-using-bedtools-and-r/



```{r ranking_orf}
writeLines("== Classement des ORFs ==")

# Top 10 des ORFs
for (i in c(1:10)) {
  print(paste0("L'ORF en ",i, " position est :"))
  print(count_ORF_ATL[which(count_ORF_ATL$nbre_SNPs == sort(count_ORF_ATL$nbre_SNPs,partial=length(count_ORF_ATL$nbre_SNPs)-i+1)[length(count_ORF_ATL$nbre_SNPs)-i+1]),])
}
