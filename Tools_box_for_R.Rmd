...
Il s'agit de la **moyenne arithmétique des varaiable quantitatives**. On untilise des vecteurs pour fournir les valeurs à la fonction.

```{r mean}

mean(c(1,9,16,84,9,354,7))
...
La médiane sert à caractériser la dispersion d’une distribution, une population, ou d’un échantillon autour de sa moyenne ou son espérance.
La médiane d’une distribution, une population, ou d’un échantillon est un valeur au centre des observations trié, ou il y a 50% des observation d'un côté, et 50% de l'autre. Elle 
...
Il s'agit ici de la **co-variance non biaisées des varaiables quantitatives**. Ici les co-variances du dataframes `donnees` sont sur la diagonale de la matrice.
