...
Pour créer un dataframe appelé `df` à partir du fichier `donneAimporter.txt`

```{r importation_dataframe}
df <- read.table("~/Documents/donneAimporter.txt", header = FALSE, dec = ".", sep = "\t", stringsAsFactors=F)
df
```

On remarque bien que :
- Le fichier à récupérer est un chaine de caractère contenant le chemin `~/Document/exemples/` et le nom du fichier `donneAimporter.txt`. 
...
```{r scan}
vect_import <- scan("~/Document/exemples/donneAimporter.txt", what = "")
vect_import
```
...
Après l'importation des données il est souvent necessaire de les transformer. Bien souvvent lorsqu'un objet est transformer il est necessaire d'assigner la modification au nom de l'objet pour le conserver.
...
```{r}
abondance_brut <- data.frame(llply(abondance_brut, function(x) {
  abondance_brut <- str_replace_all(x, pattern="[^0-9\\.\\,-]", replacement="")
...
Le `^` représente le début de l'expression régulière, et `$` représente la fin. Le `.` représente n'importe quel type de caractère et le `*` défini une répétition (de 0 à n'importe)
...
```{r}
# comparaison_F11_A-M.Rmd est le nom du fichier
rmarkdown::render("comparaison_F11_A-M.Rmd", "all") 
```
...
```bash
# Ligne de commande dans le terminal
Rscript -e "rmarkdown::render('comparaison_F11_A-M.Rmd', params=list(A = 'private_AF11-I-T48_intersec_3_replicats.csv', B = 'private_MF11-I-T48_intersec_3_replicats.csv', C = 'AF11-I-T48_intersec_3_replicats_inter_MF11-I-T48_intersec_3_replicats.csv'))"
```

L'entête du fichier  `comparaison_F11_A-M.Rmd` étant: 
